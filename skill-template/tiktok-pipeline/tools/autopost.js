...
// Your GitHub Pages root. Override with TIKTOK_BASE_URL in .env.
const DEFAULT_BASE_URL = 'https://YOURUSER.github.io/YOURREPO/';
...
function parseArgs(argv) {
  const opts = {
    ...
    description: null,
    cover: 0,
    baseUrl: process.env.TIKTOK_BASE_URL || DEFAULT_BASE_URL,
    directPost: false,
    privacy: 'SELF_ONLY',
    wait: false,
    dryRun: false,
...
Usage: node tools/autopost.js <topic> [options]
  ...
  --account NAME       key from the ACCOUNTS map below
...
async function main() {
  ...
  const ACCOUNTS = {
    // One entry per TikTok account. The env var holds that account's
    // refresh token in .env; the label is only for console output.
    main: { env: 'TIKTOK_REFRESH_TOKEN', label: '@youraccount' },
    // second: { env: 'TIKTOK_REFRESH_TOKEN_SECOND', label: '@second' },
  };
