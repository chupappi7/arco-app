...
# Paths derive from this file's location, so the pipeline works wherever the
# repo is cloned. Override SHOTS with TIKTOK_SHOTS_DIR if your app
# screenshots live outside the repo.
_HERE = os.path.dirname(os.path.abspath(__file__))
REPO = os.path.dirname(_HERE)
SP = os.path.join(_HERE, 'slides', 'bg')        # backgrounds
ICONS = os.path.join(_HERE, 'slides', 'icons')  # app icons
SHOTS = os.environ.get('TIKTOK_SHOTS_DIR', os.path.join(REPO, 'screenshots'))
...
# Your app is the deliberate constant: it appears in every listicle at #2 and is
...
# Your own app: exempt from every roster rule, always slide 2.
# Set this to your app's name and its full App Store name.
ALWAYS_ALLOWED = {'YOURAPP', 'YOURAPP: Full Store Name'}

APP_ANGLES = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                          'app_angles.json')


def next_app_angle():
    """Return the next unused body copy for your own app, cycling through all angles.

    Your app appears in every listicle, which makes its slide the most repeated
...
def assert_fresh_tools(tools, allow=()):
    """Raise if a post reuses a tool (other than your own) that is still on cooldown."""
...
# Your own app is exempt: its closing line is deliberate brand copy.
FILLER = [
...
def cta_slide(bg, out, subtitle='One line about your app.'):
    ...
    ic, mask = rounded_icon(f'{ICONS}/icon-yourapp.png', size=250, radius=56)
    ...
    draw_text_block(im, [
        (540, 940, 'YOURAPP: Full Store Name', name_f, 'ma', (255, 255, 255)),
